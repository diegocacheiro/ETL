...
public class RemoteDataService {
	...
	public void getDataFromRemote() throws UnirestException {
		...
		System.out.println(travels.get(travels.size()-1).getId());
		System.out.println(travels.get(0).getId());
				...
				if (travel.getId() > 24) {
					...
					for (int i = 0; i < locations.size(); i++) {
						...
						if (i == 0) {
							...
							for (int j = 0; j < acceletarions.size(); j++) {
								...
								if (Long.parseLong(acceletarions.get(j).getTime()) >= Long
										...
										|| Long.parseLong(acceletarions.get(j).getTime()) <= Long
												.parseLong(locations.get(i).getTime())) {
									measures.add(
											new Measure(new Timestamp(Long.parseLong(acceletarions.get(j).getTime())),
							...
							tp.add(new TrackPoint(
									...
									new Timestamp(Long.parseLong(locations.get(i).getTime())),
									new Timestamp(
											Long.parseLong(acceletarions.get(acceletarions.size() - 1).getTime())),
									measures));
						}
					}
					...
					for (int i = 0; i < bumps.size(); i++) {
						for (int j = 0; j < tp.size(); j++) {
							if (new Timestamp(Long.parseLong(bumps.get(i).getTime()))
									.equals(tp.get(j).getStartTime())) {
								Double maxVa = 0.0;
								Timestamp time = null;
					...
					System.out.println("Traza añadida");
				}
			} else {
				System.out.println("No hay trazas nuevas");
			}
		} 
	}
}
